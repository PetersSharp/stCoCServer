...
namespace stRrd.Graph
{
	internal class Grapher 
	{
		...
		private void PlotImageBackground( Graphics g )
		{
			...
			{
                Bitmap img = null;
                if ((img = graphDef.GetBackgroundBitmap()) == null)
                {
                    using (FileStream bgImage = graphDef.GetBackgroundStream())
                    {
                        if (bgImage != null)
                        {
                            img = (Bitmap)Bitmap.FromStream(bgImage);
                        }
                    }
                }
                if (img != null)
		...
		private void PlotOverlay( Graphics g )
		{
			...
			try 
			{
                using (FileStream overlayImg = graphDef.GetOverlay())
                {
                    if (overlayImg != null)
                    {
