...
namespace stCoCAPI
{
    public class ClanMemberAuth : TablePropertyMapMethod
    {
        ...
        [TablePropertyMapAttribute("alias", "STRING")]
        public string alias { get; set; }

        [TablePropertyMapAttribute("passwd", "STRING")]
        public string passwd { get; set; }

        [TablePropertyMapAttribute("passwdhash", "STRING")]
        public string passwdhash { get; set; }

        [TablePropertyMapAttribute("approve", typeof(System.Int32))]
        public int approve { get; set; }

        [TablePropertyMapAttribute("dtreg", "DATETIME")]
        public string dtreg { get; set; }

        [TablePropertyMapAttribute("dtapprove", "DATETIME")]
        public string dtapprove { get; set; }
    ...
    /// see client side <see cref="stCoCAPI.Client"/>
    /// </summary>
    public static class SQLViewRequest
    {
        ...
        const string ViewClanMember =
            ...
            "dtin, " +
            "dtout, " +
            "AllLeague.id AS leagueid, " +
            "AllLeague.name AS league, " +
            "COCLEAGUEICO(AllLeague.ico) AS ico " +
            "FROM ClanMember " +
        ...
        const string ViewClanMemberAuth =
            ...
            "ClanMember.status AS status, " +
            "ClanMember.year AS year, " +
            "ClanMember.season AS season, " +
            "ClanMember.tag AS tag, " +
            "ClanMember.nik AS nik, " +
            "ClanMemberAuth.alias AS alias, " +
            "ClanMember.league AS leagueid, " +
            "COCPASSWORD(ClanMemberAuth.tag, ClanMemberAuth.passwd) AS passwd " +
            "FROM ClanMember " +
